...
def get_errors(error_string):
    return u' '.join(
        ...
        #line for line in error_string.splitlines()
    ).strip()
...
def run_tesseract_v2(input_data,
                  extension,
                  lang,
                  config='',
                  nice=0,
    ...
    with timeout_manager(proc, timeout) as error_string:
        if proc.returncode:
            ...
            print(error_string)
            raise TesseractError(proc.returncode, get_errors(error_string.decode("utf-8")))
...
def run_and_get_output_v1(image,
                       extension='',
                       lang=None,
                       config='',
                       nice=0,
...
def run_and_get_output_v2(image,
                       extension='',
                       lang=None,
                       config='',
                       nice=0,
        ...
        result = run_tesseract_v2(**kwargs)
        if return_bytes:
            return result
...
def image_to_string_v1(image,
    ...
    return {
        Output.BYTES: lambda: run_and_get_output_v1(*(args + [True])),
        Output.DICT: lambda: {'text': run_and_get_output_v1(*args)},
        Output.STRING: lambda: run_and_get_output_v1(*args),
    }[output_type]()
...
def image_to_string(image,
    ...
    args = [bmp, 'txt', lang, config, nice, timeout]

    return {
        Output.BYTES: lambda: run_and_get_output_v2(*(args + [True])),
        Output.DICT: lambda: {'text': run_and_get_output_v2(*args)},
        Output.STRING: lambda: run_and_get_output_v2(*args),
    }[output_type]()


def image_to_pdf_or_hocr_v1(image,
                         lang=None,
                         config='',
                         nice=0,
                         extension='pdf',
    ...
    return run_and_get_output_v1(*args)


def image_to_boxes_v1(image,
                   lang=None,
                   config='',
                   nice=0,
                   output_type=Output.STRING,
...
def image_to_boxes(image,
    ...
    config += ' batch.nochop makebox'
    _, bmp = cv2.imencode('.bmp', image)
    args = [bmp, 'box', lang, config, nice, timeout]

    return {
        Output.BYTES: lambda: run_and_get_output_v2(*(args + [True])),
        Output.DICT: lambda: file_to_dict(
            'char left bottom right top page\n' + run_and_get_output_v2(*args),
            ' ',
            0),
        Output.STRING: lambda: run_and_get_output_v2(*args),
    }[output_type]()
...
def get_pandas_output_v1(args):
    if not pandas_installed:
        raise PandasNotSupported()

    return pd.read_csv(
        BytesIO(run_and_get_output_v1(*args)),
        quoting=QUOTE_NONE,
        sep='\t'
    )
...
def get_pandas_output_v2(args):
    if not pandas_installed:
        raise PandasNotSupported()

    return pd.read_csv(
        BytesIO(run_and_get_output_v2(*args)),
        quoting=QUOTE_NONE,
        sep='\t'
    )


def image_to_data_v1(image,
                  lang=None,
                  config='',
                  nice=0,
                  output_type=Output.STRING,
    ...
    return {
        Output.BYTES: lambda: run_and_get_output_v1(*(args + [True])),
        Output.DATAFRAME: lambda: get_pandas_output_v1(args + [True]),
        Output.DICT: lambda: file_to_dict(run_and_get_output_v1(*args), '\t', -1),
        Output.STRING: lambda: run_and_get_output_v1(*args),
    }[output_type]()
...
def image_to_data(image,
    ...
    config = '{} {}'.format('-c tessedit_create_tsv=1', config.strip()).strip()
    _, bmp = cv2.imencode('.bmp', image)
    args = [bmp, 'tsv', lang, config, nice, timeout]

    return {
        Output.BYTES: lambda: run_and_get_output_v2(*(args + [True])),
        Output.DATAFRAME: lambda: get_pandas_output_v2(args + [True]),
        Output.DICT: lambda: file_to_dict(run_and_get_output_v2(*args), '\t', -1),
        Output.STRING: lambda: run_and_get_output_v2(*args),
    }[output_type]()


def image_to_osd_v1(image,
                 lang='osd',
                 config='',
                 nice=0,
                 output_type=Output.STRING,
    ...
    return {
        Output.BYTES: lambda: run_and_get_output_v1(*(args + [True])),
        Output.DICT: lambda: osd_to_dict(run_and_get_output_v1(*args)),
        Output.STRING: lambda: run_and_get_output_v1(*args),
    }[output_type]()
